...
const onLoad = () => {
  ...
  const pagesVideoData = []

  intervalId = window.setInterval(function () {
    tryReadVideoLength(allVideos)
  }, 3000)
}
...
function tryReadVideoLength(allVideos) {
  try {
    allVideos[1]
      .querySelectorAll("ytd-thumbnail-overlay-time-status-renderer")[0]
      .innerHTML.trim()
    gotMessage()
...
function addVideosToBackend(videosDataList) {
  ...
  fetch("http://127.0.0.1:5000/videosReceiver", {
    ...
    // Strigify the payload into JSON:
    body: JSON.stringify(videosDataList)
  })
    .then((res) => {
      if (res.ok) {
        return res.json()
...
function getDataFromVideo(video, orderOnScreen) {
  ...
  try {
    ...
    let channelName = video.querySelectorAll(
      "yt-formatted-string.ytd-channel-name"
    )[0].innerText
    let videoLength = video
      .querySelectorAll("ytd-thumbnail-overlay-time-status-renderer")[0]
      .innerText.trim()
